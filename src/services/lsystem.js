...
TurtleRenderer.prototype =
    {
         ...
         * @return {TurtleRenderer} returns 'this' for method chaining
         */
         ...
         * @return {Dimension} representing the min/max coordinate values.
         */
        getMinMaxValues: function getMinMaxValues() {
            return new Dimension(this._minx, this._miny, this._maxx, this._maxy);
        },
        ...
        process: function process(cmds, draw) {
            ...
            var pos = new Location(0.0, 0.0, 90.0, -1);
 ...
 * 
 * @namespace LSystems
 * @class LSystemsProcessor
 */
const LSystemsProcessor = function () {
    this.rules = [];
    return this;
};

LSystemsProcessor.prototype =
    {
        /**
 ...
 * @class Location
 */
const Location = function (x, y, heading, colour) {
    this.x = x;
    this.y = y;
 ...
 * @class Dimension
 */
const Dimension = function (minx, miny, maxx, maxy) {
    this.minx = minx;
    this.miny = miny;
    this.maxx = maxx;
    this.maxy = maxy;
...
const run = (options) => {
    ...
    try {
        ...
        const oldDistance = 10.0;
        let newDistance;
        let dim = renderer.getMinMaxValues();;
        ...
        const xoffset = (WIDTH / 2) - (((dim.maxx - dim.minx) / 2) + dim.minx);
        const yoffset = (HEIGHT / 2) - (((dim.maxy - dim.miny) / 2) + dim.miny);
        ...
        console.error("Error during LSystemsProcessor.generate()", e);
